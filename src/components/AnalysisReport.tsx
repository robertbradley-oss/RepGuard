...
export function AnalysisReport({ report, status }: AnalysisReportProps) {
  ...
  const checkStyles = {
    Clear: "bg-[#E9FFF0] text-[#0B5F2A] ring-[#41D66F]/40",
    Review: "bg-[#F7FBFF] text-[#066B8F] ring-[#19D3F3]/45",
  ...
  const signalStyles = {
    ...
    Inconclusive: "bg-[#F7FBFF] text-[#066B8F] ring-[#19D3F3]/45",
    "Low confidence": "bg-[#F7FBFF] text-slate-600 ring-[#E4F0F7]",
    "No material signal": "bg-[#E9FFF0] text-[#0B5F2A] ring-[#41D66F]/40",
  };
    ...
    <section className="cg-panel rounded-lg p-4">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-1 text-lg font-semibold text-[#061426]">Evidence summary</h2>
        </div>
        <span className="rounded-md border border-[#E4F0F7] bg-[#F8FCFF] px-2.5 py-1 text-xs font-semibold text-slate-600">
          {isComplete ? report.reviewLabel : status === "analyzing" ? "Analyzing" : "Awaiting evidence"}
        </span>
      </div>

      {!isComplete ? (
        <div className="mt-5 rounded-lg border border-dashed border-[#D5E8F3] bg-[#F8FCFF] p-5">
          <div className="h-2 overflow-hidden rounded-full bg-[#E4F0F7]">
            <div
              className={`h-full rounded-full cg-brand-gradient ${status === "analyzing" ? "w-2/3 animate-pulse" : "w-1/4"}`}
            />
          </div>
      ...
      {isComplete ? (
        <div className="mt-4 space-y-4">
          <div className="rounded-lg border border-[#E4F0F7] bg-[#F8FCFF] p-4">
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <article className="rounded-lg border border-[#E4F0F7] bg-white p-3 shadow-[0_8px_20px_rgba(6,20,38,0.025)]">
              <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                <Gauge className="size-4 text-[#08AEEA]" aria-hidden="true" />
                Authenticity score
              </div>
              <p className="mt-2 text-2xl font-bold text-[#061426]">{report.score}</p>
            ...
            <article className="rounded-lg border border-[#E4F0F7] bg-white p-3 shadow-[0_8px_20px_rgba(6,20,38,0.025)]">
              <div className="flex items-center gap-2 text-sm font-semibold text-slate-900">
                <ShieldQuestion className="size-4 text-[#00A7A5]" aria-hidden="true" />
                Confidence level
              </div>
              <p className="mt-2 text-lg font-semibold text-[#061426]">{report.confidenceLevel}</p>
            ...
            <article className="rounded-lg border border-[#E4F0F7] bg-white p-3 shadow-[0_8px_20px_rgba(6,20,38,0.025)]">
              ...
              <p className="mt-2 text-lg font-semibold text-[#061426]">{report.riskLevel} risk</p>
              <p className="mt-2 text-sm leading-5 text-slate-600">
          ...
          <div className="rounded-lg border border-[#E4F0F7] bg-white p-4">
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <article className="rounded-lg border border-[#E4F0F7] bg-white p-4">
          <ShieldQuestion className="size-5 text-[#08AEEA]" aria-hidden="true" />
        ...
        <article className="rounded-lg border border-[#E4F0F7] bg-white p-4">
          <CheckCircle2 className="size-5 text-[#41D66F]" aria-hidden="true" />
        ...
        <article className="rounded-lg border border-[#E4F0F7] bg-white p-4">
          <MessageSquareText className="size-5 text-[#08AEEA]" aria-hidden="true" />
      ...
      {isComplete ? (
        <div className="mt-4 space-y-4">
          <section className="rounded-lg border border-[#E4F0F7] bg-white">
            <div className="border-b border-[#E4F0F7] px-4 py-3">
            ...
            <div className="grid gap-3 p-4 xl:grid-cols-2">
              {report.riskSignalGroups.map((group) => {
                ...
                return (
                  <article className="rounded-lg border border-[#E4F0F7] bg-[#F8FCFF] p-3" key={group.category}>
                    <div className="flex items-start gap-3">
                      <span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-white text-[#08AEEA] ring-1 ring-[#DDECF5]">
                    ...
                    <div className="mt-3 space-y-3">
                      {group.signals.map((signal) => (
                        <div className="rounded-lg bg-white p-3 ring-1 ring-[#E4F0F7]" key={signal.label}>
          ...
          <section className="rounded-lg border border-[#E4F0F7] bg-white">
            <div className="border-b border-[#E4F0F7] px-4 py-3">
              <div className="flex items-center gap-2">
                <ListChecks className="size-4 text-[#00A7A5]" aria-hidden="true" />
                <h3 className="text-sm font-semibold text-slate-900">What to verify next</h3>
              </div>
            </div>
            <div className="grid gap-3 p-4 md:grid-cols-3">
              {report.whatToVerifyNext.map((item) => (
                <div className="flex gap-3 rounded-lg bg-[#F8FCFF] p-3 ring-1 ring-[#E4F0F7]" key={item}>
          ...
          <section className="rounded-lg border border-[#E4F0F7] bg-white">
            <div className="border-b border-[#E4F0F7] px-4 py-3">
              <div className="flex items-center gap-2">
                <TicketCheck className="size-4 text-[#08AEEA]" aria-hidden="true" />
          ...
          <div className="rounded-lg border border-[#E4F0F7] bg-[#F8FCFF] p-4 text-sm leading-6 text-slate-600">
