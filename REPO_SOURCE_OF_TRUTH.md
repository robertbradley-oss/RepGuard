...
- Phase 4.5 planning-only server-side OCR route skeleton implementation planning is documented in `PHASE_4_5_SERVER_SIDE_OCR_ROUTE_SKELETON_PLAN.md`. It prepares a future `POST /api/analysis/ocr` skeleton that must remain JSON-only, synthetic-only, mock-only, provider-free, SDK-free, env-free, upload-free, storage-free, persistence-free, UI-free, live-analyzer-free, and receipt-behavior-free. It defines allowlisted fixture-key input, real-file/customer-identifier rejection, output through the Phase 4.2 fixture harness and Phase 4.3 extraction contract, validation/failure behavior, isolation rules, probes/checks, safety wording, privacy rules, and Phase 4.6 gates. It does not implement a route or change runtime behavior.
- The next safe task is Phase 4.6 synthetic-only/mock-provider server-side OCR route skeleton implementation, only if Robert explicitly opens that implementation slice. Do not implement real AI, OCR provider calls, provider SDKs, credentials, env vars, live provider routes, upload behavior, persistence, live scoring, real evidence handling, `ClaimReviewWorkflow` wiring, `ProductPhotoReviewPanel` routing, `LocalAnalysisResult` migration, or receipt behavior changes until Robert explicitly opens a named live/provider implementation slice.
