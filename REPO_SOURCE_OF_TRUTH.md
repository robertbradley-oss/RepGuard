...
- Phase 2.8.2 is pushed at `7ee4b65` (`feat: add dev unsupported evidence review bridge`) as a dev-only unsupported-evidence review bridge at `src/app/dev/unsupported-evidence-review/page.tsx` with colocated literal synthetic cases in `render-cases.ts` and semantic/import/privacy guards in `scripts/check-report-semantics.mjs`. It remains unlinked, production-disabled, synthetic-only, and isolated from live runtime, upload, report mapping, parser, scoring, fixtures, `ClaimReviewWorkflow`, `ProductPhotoReviewPanel`, providers/storage/integrations/case queues, OCR/metadata, `analyzeEvidenceFile`, and `LocalAnalysisResult`.
- A follow-up Phase 2.8.2 closeout blocker patch removes unsafe rendered score-boundary wording from the dev bridge and strengthens semantic coverage for simple split/joined visible strings. The next safest move is review and, if approved, commit that blocker patch. Do not wire the wrapper into live callers or change runtime/UI/upload/report behavior without explicit approval.
