...
Phase 4.0 planning-only real AI/OCR/photo intelligence readiness is documented in `docs/archive/phase-4/PHASE_4_0_AI_OCR_PHOTO_INTELLIGENCE_READINESS.md`. Phase 4.1 docs-only OCR/provider architecture planning is documented in `docs/archive/phase-4/PHASE_4_1_OCR_PROVIDER_ARCHITECTURE_PLAN.md`. Phase 4.2 non-live synthetic OCR fixture harness is implemented in `src/lib/analysis/ocr-fixture-harness.ts` with probe coverage in `src/lib/analysis/ocr-fixture-harness.probe.ts`. Phase 4.3 non-live provider-neutral OCR extraction contract is implemented in `src/lib/analysis/ocr-extraction-contract.ts` with probe coverage in `src/lib/analysis/ocr-extraction-contract.probe.ts`. Phase 4.4 planning-only server-side OCR route and data-flow design is documented in `docs/archive/phase-4/PHASE_4_4_SERVER_SIDE_OCR_ROUTE_DATA_FLOW_PLAN.md`. Phase 4.5 planning-only server-side OCR route skeleton implementation planning is documented in `docs/archive/phase-4/PHASE_4_5_SERVER_SIDE_OCR_ROUTE_SKELETON_PLAN.md`. Phase 4.6 synthetic-only/mock-provider server-side OCR route skeleton is implemented at `src/app/api/analysis/ocr/route.ts` with route probe coverage in `src/app/api/analysis/ocr/route.probe.ts`. Phase 4.7 OCR route safety/readiness checkpoint is documented in `docs/archive/phase-4/PHASE_4_7_OCR_ROUTE_SAFETY_READINESS_CHECKPOINT.md`; the route now accepts exactly a `fixtureKey` request field and remains covered by route/probe semantic checks. Phase 4.8 OCR route hardening and developer-only fixture testing documentation is documented in `docs/archive/phase-4/PHASE_4_8_OCR_ROUTE_HARDENING_FIXTURE_TESTING.md`; the route now carries a source-level developer warning that is verified by route/probe semantic checks. Phase 4.9 provider selection planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_9_OCR_PROVIDER_SELECTION_PLAN.md`; it recommends OCR-specialized extraction first for receipt fields, OpenAI Vision-style reasoning later for screenshots/layout/product-photo/photo-intelligence uncertainty signals, local OCR as a development/fallback baseline, a hybrid pipeline as the staged target, and ClaimGuard's internal contract as source of truth. Phase 4.10 provider abstraction planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_10_PROVIDER_ABSTRACTION_PLAN.md`; it defines future provider categories, adapter concepts, OCR/vision result shapes, failure normalization, privacy/redaction, cost/timeout, and mock-before-live rules without implementation. Phase 4.11 mock provider adapter planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_11_MOCK_PROVIDER_ADAPTER_PLAN.md`; it defines future mock OCR, mock vision reasoning, failure, timeout, unsupported, cost/usage, redaction-status, module-boundary, input/output, privacy, probe, and safety gates. Phase 4.12 mock provider adapter skeleton is implemented in `src/lib/analysis/providers/mock-provider-adapter.ts` with probe coverage in `src/lib/analysis/providers/mock-provider-adapter.probe.ts`; it remains synthetic-only, mock-only, provider-free, SDK-free, env-free, upload-free, storage-free, persistence-free, UI-free, live-analyzer-free, real-evidence-free, receipt-behavior-free, and unwired from the existing OCR route and live receipt workflow. Phase 4.13 mock provider adapter safety/readiness checkpoint is documented in `docs/archive/phase-4/PHASE_4_13_MOCK_PROVIDER_ADAPTER_SAFETY_READINESS_CHECKPOINT.md` with semantic coverage. Phase 4.14 mock provider adapter developer usage documentation is documented in `docs/archive/phase-4/PHASE_4_14_MOCK_PROVIDER_ADAPTER_DEVELOPER_USAGE.md` with semantic coverage. Phase 4.15 mock adapter route integration planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_15_MOCK_ADAPTER_ROUTE_INTEGRATION_PLAN.md` with semantic coverage. Phase 4.16 separate mock provider route skeleton is implemented at `src/app/api/analysis/mock-provider/route.ts` with route probe and semantic coverage; it calls only the mock provider adapter and keeps the existing OCR route exact `fixtureKey` only. Phase 4.17 mock provider route safety/readiness checkpoint is documented in `docs/archive/phase-4/PHASE_4_17_MOCK_PROVIDER_ROUTE_SAFETY_READINESS_CHECKPOINT.md` with semantic coverage. Phase 4.18 mock provider route developer usage documentation is documented in `docs/archive/phase-4/PHASE_4_18_MOCK_PROVIDER_ROUTE_DEVELOPER_USAGE.md` with semantic coverage. Phase 4.19 OpenAI Vision sandbox planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_19_OPENAI_VISION_SANDBOX_PLAN.md` with semantic coverage. Phase 4.20 OpenAI Vision sandbox prompt/output contract planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_20_OPENAI_VISION_PROMPT_OUTPUT_CONTRACT_PLAN.md` with semantic coverage; it defines future prompt template families, structured output fields, altered-or-AI-generated-image uncertainty wording, observation-vs-conclusion separation, refusal/unsupported handling, provider failure/timeout/cost handling, privacy-safe prompt/output constraints, and future QA gates without implementation. Phase 4.21 OpenAI Vision sandbox schema planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_21_OPENAI_VISION_SANDBOX_SCHEMA_PLAN.md` with semantic coverage; it defines future schema identity, evidence classification, visual summaries, observations, uncertainty signals, altered-or-AI-generated-image uncertainty, confidence notes, manual-review drivers, limitations, privacy/retention metadata, cost/timeout metadata, failure shapes, schema validation, and QA gates without implementation. Phase 4.22 OpenAI Vision sandbox fixture-policy planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_22_OPENAI_VISION_SANDBOX_FIXTURE_POLICY_PLAN.md` with semantic coverage; it defines future synthetic fixture categories, redacted/anonymized fixture approval gates, disallowed fixture categories, naming/metadata rules, scenario matrix, altered-or-AI-generated-image uncertainty fixture rules, privacy/retention rules, QA/probe requirements, route/mock-adapter/schema relationships, and approval gates without adding fixtures or implementation. Phase 4.23 OpenAI Vision sandbox validation/probe planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_23_OPENAI_VISION_SANDBOX_VALIDATION_PROBE_PLAN.md` with semantic coverage; it defines future validation categories, probe categories, safety wording scans, altered-or-AI-generated-image uncertainty validation, observation-vs-signal separation validation, privacy/identifier scans, fixture-policy enforcement validation, unsupported/failure-state probes, schema/output probes, route/runtime/package guard scans, downloadable-package safety checks, execution strategy, relationships to existing routes/plans, and approval gates without adding validation/probe implementation beyond narrow semantic checker coverage. Phase 4.24 synthetic fixture metadata schema planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_24_SYNTHETIC_FIXTURE_METADATA_SCHEMA_PLAN.md` with semantic coverage; it defines future metadata purpose/scope, identity fields, allowed/disallowed category and evidence enums, prompt/schema linkage fields, expected-output metadata, altered-or-AI-generated-image uncertainty metadata, privacy/identifier/redaction fields, retention/ownership fields, package-distribution safety fields, cost/timeout expectations, required vs optional field groups, validation expectations, route/mock-adapter/fixture-policy/validation-plan relationships, approval gates, and Phase 4.25 options without adding metadata schema/types, fixtures/images, validators/probes beyond narrow semantic checker coverage, SDKs, env vars, provider calls, route behavior changes, uploads, storage, persistence, UI, package artifacts, real evidence, or live AI/Vision/photo analysis. Phase 4.25 validation/probe implementation planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_25_VALIDATION_PROBE_IMPLEMENTATION_PLAN.md` with semantic coverage; it defines the Phase 4.26 local static validation/probe implementation sequence, protected files, blocked patterns, no-SDK/env/provider-call checks, no-upload/storage/object URL checks, runtime/route boundary checks, privacy/identifier checks, provider-payload/raw-OCR checks, altered-or-AI-generated-image uncertainty wording checks, observation-vs-signal checks, fixture metadata policy checks, package-safety checks, expected script names, and approval gates without adding validation/probe implementation beyond narrow semantic checker/source-of-truth coverage. Phase 4.26 local static sandbox validation/probe implementation is documented in `docs/archive/phase-4/PHASE_4_26_SANDBOX_VALIDATION_PROBES.md` and implemented by `scripts/check-vision-sandbox-boundaries.mjs` with npm script `check:vision-sandbox-boundaries`; it remains local-static-only and unwired from runtime. Phase 4.27 synthetic fixture metadata is documented in `docs/archive/phase-4/PHASE_4_27_SYNTHETIC_FIXTURE_METADATA.md` and implemented in `fixtures/vision-sandbox/metadata/synthetic-fixture-registry.json`; it adds package-safe metadata only and no images/evidence files. Phase 4.28 synthetic fixture creation planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_28_SYNTHETIC_FIXTURE_CREATION_PLAN.md` with semantic coverage and creates no fixtures. Phase 4.29 synthetic vision fixture creation is documented in `docs/archive/phase-4/PHASE_4_29_SYNTHETIC_VISION_FIXTURES.md` and implemented under `fixtures/vision-sandbox/assets/` and `fixtures/vision-sandbox/simulations/`. Phase 4.30 OpenAI Vision sandbox skeleton planning-only documentation is documented in `docs/archive/phase-4/PHASE_4_30_OPENAI_VISION_SANDBOX_SKELETON_PLAN.md`; it defines future developer-only skeleton purpose, module boundaries, route boundary options, input/output boundaries, fixture/metadata relationships, validation/probe requirements, existing route/mock-adapter relationships, package/downloadable safety, Phase 4.31 recommendation, and stop conditions without implementing skeleton code, routes, runtime schema/types, fixtures, SDKs, env vars, provider calls, uploads, storage, persistence, UI, package artifacts, real evidence, receipt behavior changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes. Real OCR/provider implementation has not started. Stronger OCR, AI/vision, and photo intelligence should remain evidence review signals with confidence, uncertainty, limitations, and manual-review guidance, not final verdicts, fraud conclusions, or automated support decisions.

Phase 4.31 OpenAI Vision sandbox skeleton implementation is complete as script/module-only developer sandbox work under `src/lib/analysis/vision-sandbox/` with `check:vision-sandbox-skeleton`. It resolves approved synthetic fixture metadata/assets, builds deterministic sandbox-shaped stubs, simulates unsupported/failure states, exposes schema/privacy/package-safety guards, and adds no route behavior, SDK/env/provider calls, uploads, storage, persistence, real evidence, fixture changes, UI wiring, receipt behavior changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes. Phase 4.32 OpenAI Vision sandbox fixture-runner validation is complete as script/module-only hardening under `src/lib/analysis/vision-sandbox/fixture-runner.ts` with `check:vision-sandbox-fixture-runner`; it validates all approved synthetic fixture references, mode fallback behavior, failure simulations, guard states, privacy/retention markers, and package-safety markers while adding no route behavior, SDK/env/provider calls, uploads, storage, persistence, real evidence, fixture changes, UI wiring, receipt behavior changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes. Phase 4.33 OpenAI Vision sandbox developer usage/readiness is complete in `docs/archive/phase-4/PHASE_4_33_VISION_SANDBOX_DEVELOPER_USAGE_READINESS.md`; it documents safe commands, capabilities, non-capabilities, fixture/metadata/package safety, safety language, protected boundaries, readiness for provider-configuration planning only, and stop conditions before API-credit-using work. Phase 4.34 OpenAI Vision provider configuration planning is complete in `docs/archive/phase-4/PHASE_4_34_OPENAI_VISION_PROVIDER_CONFIG_PLAN.md`; it documents future provider configuration goals, explicit opt-in and disabled-by-default rules, API-credit usage boundaries, future `.env.example` guidance only, model/provider selection strategy, timeout/retry/rate/cost defaults, privacy and retention defaults, sandbox relationship, downloadable/self-hosted package safety, approval gates, and Phase 4.35 safe options. Phase 4.35 real OpenAI Vision sandbox implementation planning is complete in `docs/archive/phase-4/PHASE_4_35_OPENAI_VISION_SANDBOX_IMPLEMENTATION_PLAN.md`; it documents exact future file boundaries, SDK versus minimal fetch adapter tradeoffs, provider request/response flow, synthetic-only first-call policy, API-credit approval policy, env/config requirements, prompt/schema/fixture-runner integration, package safety, future checks, and stop conditions. Phase 4.36 OpenAI Vision sandbox approval checkpoint is complete in `docs/archive/phase-4/PHASE_4_36_OPENAI_VISION_SANDBOX_APPROVAL_CHECKPOINT.md`; it separates Robert approval choices for provider configuration skeleton, API-credit-using synthetic fixture calls, pausing live provider work, and downloadable/self-hosted setup planning, and confirms only explicit API-credit approval authorizes credit-using work. Phase 4.37 OpenAI Vision provider configuration skeleton is complete in `docs/archive/phase-4/PHASE_4_37_OPENAI_VISION_PROVIDER_CONFIG_SKELETON.md`; it adds inert sandbox-only disabled defaults under `src/lib/analysis/vision-sandbox/provider-config.ts` without `.env.example`, SDKs, provider calls, API-credit behavior, routes, uploads, storage, persistence, real evidence, receipt changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes. Phase 4.38 provider configuration skeleton hardening and `.env.example`/package-safety review is complete and still non-live. Phase 4.39 provider configuration developer usage documentation and API-credit approval readiness is complete in `docs/archive/phase-4/PHASE_4_39_PROVIDER_CONFIG_DEVELOPER_USAGE_AND_API_CREDIT_READINESS.md`. Phase 4.40 API-credit approval decision and first live sandbox call planning is complete in `docs/archive/phase-4/PHASE_4_40_API_CREDIT_APPROVAL_AND_FIRST_LIVE_SANDBOX_CALL_PLAN.md`; it does not approve Choice B, and the recommended next path is conditional Phase 4.41: first API-credit-using synthetic-only sandbox implementation only if Robert explicitly approves Choice B in plain language, otherwise provider config/checker hardening with still no API credits.
...
- `docs/qa/TEST_EVIDENCE.md`: manual QA and fixture guidance.
...
- Phase 4.38 checkpoint: OpenAI Vision provider configuration skeleton hardening and `.env.example`/package-safety review is complete; provider remains disabled by default, provider calls and request execution remain blocked, API-credit usage remains blocked, evidence scope remains synthetic-fixture-only, payload logging and raw OCR retention remain disabled, and `.env.example` is safe, disabled, blank for provider key, and example-only. No live OpenAI implementation, SDK/provider call, API-credit behavior, route behavior, uploads, storage, persistence, real evidence, fixture changes, UI wiring, receipt behavior changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes were added. Recommended next task: Phase 4.39 provider configuration developer usage documentation and API-credit approval readiness only.
- Phase 4.39 checkpoint: OpenAI Vision provider configuration developer usage documentation and API-credit approval readiness is complete; safe `.env.example` usage, disabled provider config defaults, allowed/blocked actions without Choice B, API-credit readiness, package/downloadable safety, and first API-credit-using prerequisites are documented. No live OpenAI implementation, SDK/provider call, API-credit behavior, route behavior, uploads, storage, persistence, real evidence, fixture changes, UI wiring, receipt behavior changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes were added. Recommended next task: Phase 4.40 API-credit usage approval decision and first live sandbox call plan only.
- Phase 4.40 checkpoint: API-credit approval decision and first live sandbox call planning is complete; Choice B is defined but not approved by this document, the preferred future first fixture is `synthetic-clean-receipt-baseline`, and provider config/API-credit/output-validation/rollback/package-safety requirements are documented. No live OpenAI implementation, SDK/provider call, API-credit behavior, route behavior, uploads, storage, persistence, real evidence, fixture changes, UI wiring, receipt behavior changes, `analyzeEvidenceFile` changes, or `LocalAnalysisResult` changes were added. Recommended next task: conditional Phase 4.41, with API-credit-using implementation only if Robert explicitly approves Choice B in plain language; otherwise provider config/checker hardening only.
