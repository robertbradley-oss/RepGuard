...
1. Pause after Phase 2.6 closeout unless Robert explicitly opens the next scoped task. The next safest task is a review-only or docs-only next-phase scope decision; do not start the next phase, wire the wrapper into live callers, edit upload/UI/live report adapter mapping, migrate `LocalAnalysisResult`, add product-photo analyzer/report behavior, route to `ProductPhotoReviewPanel`, add providers/storage/integrations/case queues, add real photos/metadata fixtures, deploy, or push without explicit approval.
...
```text
/claimguardagent run a review-only next-phase readiness recommendation after Phase 2.6 closeout; inspect source-of-truth docs and recommend one narrow next scoped task, but do not edit files, implement, wire runtime, commit, push, deploy, or start the next phase.
```
