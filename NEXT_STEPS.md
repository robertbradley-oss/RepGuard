...
1. Open Phase 2.8.0 as a docs-only acceleration plan toward dev-only harness/UI readiness for the existing unsupported-evidence and workflow-boundary probes. Keep it high-gear but non-live: define the fastest safe bridge from probe-only state to a dev-only/user-visible readiness surface, required checks, browser/QA gates, privacy constraints, and stop conditions. Do not start implementation, wire live callers, add unsupported-evidence UI to the main workflow, edit upload/live report adapter mapping, migrate `LocalAnalysisResult`, change receipt behavior, route to `ProductPhotoReviewPanel`, add providers/storage/integrations/case queues, add real photos/metadata fixtures, deploy, or push without explicit approval.
...
```text
/claimguardagent create Phase 2.8.0 docs-only acceleration plan toward dev-only harness/UI readiness for unsupported-evidence workflow-boundary review; do not implement runtime, wire ClaimReviewWorkflow, add live unsupported-evidence UI, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, start Phase 2.9, push, or deploy
```
