...
```text
/claimguardagent run Phase 4.11 as mock provider adapter planning only for future OCR/vision providers: define the synthetic/mock adapter cases, mock result/failure/timeout/cost fixtures, semantic/probe coverage, privacy/redaction markers, and protected-file gates before any implementation; do not add live providers, SDKs, env vars, provider abstraction implementation unless explicitly approved as a mock-only skeleton, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR
```
