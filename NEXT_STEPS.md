...
```text
/claimguardagent open Phase 2.3.5 as a no-live-wiring product-photo analyzer readiness hardening slice: keep analyzeEvidenceFile receipt-only and unchanged; keep LocalAnalysisResult unchanged; do not touch upload, UI insertion, live report adapter mapping, receipt parser/scoring/fixtures, providers, storage, integrations, or case queues; derive or clamp score/confidence/review-priority/local-signal overrides, add runtime enum/string canonicalizers for product-photo inputs, make product-photo probes executable or otherwise enforce their assertions, document/quarantine the legacy live damage-photo path from canonical product-photo, run lint/build/report-semantics/diff-check/import-boundary scans, commit only if safe, do not push
```
