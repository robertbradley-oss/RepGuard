...
```text
/claimguardagent prepare Phase 4.25 OpenAI Vision sandbox validation/probe implementation plan only: define the future implementation sequence and guard coverage, still no validators/probes implemented, no runtime schema/types, no fixture files/images, no SDKs/env vars/providers/uploads/storage/persistence/UI/live analyzer wiring, no real evidence, no existing route changes, no package artifacts, and no live AI/Vision/photo analysis
```
