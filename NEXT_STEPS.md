...
1. Run Phase 2.6.2 only after Robert explicitly opens it: a default-off, unwired guard-only pre-analysis gate wrapper/probe implementation per `PHASE_2_6_GATE_WIRING_DESIGN_SPIKE.md`. The slice should add only the thin wrapper, privacy-safe File-to-hints adapter, additive `UnsupportedEvidenceResult`, default-off flag, probe registration, semantic/import/privacy guards, and status docs. It must keep `analyzeEvidenceFile`, `LocalAnalysisResult`, receipt behavior, upload/UI/live report adapter mapping, product-photo analyzer/routing adapter, `ProductPhotoReviewPanel`, providers, storage, integrations, case queues, real photos, real metadata fixtures, deployment, and push work blocked unless Robert separately approves them.
