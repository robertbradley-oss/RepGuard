...
1. Run Phase 2.6.1 only after Robert explicitly opens it: a docs-only guard-only gate-wiring design spike per `PHASE_2_6_RUNTIME_SCOPE_PLAN.md` (resolve the gate branch location, the privacy-safe `File`→hints adapter, the additive unsupported/quarantine result that avoids `LocalAnalysisResult` migration, the default-off flag, and the exact receipt-preservation + gate-wiring probes and allowed/protected files for a future guard-only implementation). Alternatively, pause and run a recorded GUI multi-viewport browser-QA pass on the two existing dev harnesses. Either path keeps product-photo runtime non-live and the gate/adapter decision-only and unwired, and does not touch upload, UI, live report mapping, `analyzeEvidenceFile`, `LocalAnalysisResult`, receipt behavior, providers, storage, integrations, case queues, or deployment. Do not begin any guard-only implementation (Phase 2.6.2) until the 2.6.1 design spike is accepted and Robert names the allowed/protected files.
