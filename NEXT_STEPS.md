...
- Phase 2.4.3 is a docs-only dev-only adapter review harness plan in `docs/archive/phase-2/PRODUCT_PHOTO_DEV_HARNESS_PLAN.md`. It plans a future synthetic-only display surface for adapter readiness outputs only; it does not implement a route, component, harness, runtime adapter execution, upload path, UI insertion, report mapping, provider, storage, integration, case queue, real photo, or real metadata fixture.
- Phase 2.4.4 closes adapter readiness planning as complete enough for the non-live checkpoint and records the next runtime blockers in `docs/archive/phase-2/PRODUCT_PHOTO_RUNTIME_BLOCKERS_PLAN.md`. The main blocker is the legacy live receipt-era `damage-photo` filename classification path, which must be quarantined or migrated before any product-photo runtime, upload, UI, live report mapping, `analyzeEvidenceFile`, `LocalAnalysisResult`, provider, storage, integration, or case workflow work.
- Phase 2.4.5 is complete as a docs-only legacy `damage-photo` quarantine/migration plan in `docs/archive/phase-2/LEGACY_DAMAGE_PHOTO_QUARANTINE_PLAN.md`. It chooses the safest first future hardening boundary: filename/evidence-type classification before the live receipt-shaped analyzer path proceeds. The plan keeps `product-photo` canonical, treats `damage-photo` as legacy receipt-era/mock compatibility only, preserves receipt behavior, and defines allowed/protected files, semantic/probe/browser/privacy gates, stop conditions, and the recommended Phase 2.4.6 milestone.
...
- Phase 2.4.9 is complete as docs-only pre-OCR/pre-metadata unsupported-boundary planning in `docs/archive/phase-2/PRODUCT_PHOTO_UNSUPPORTED_BOUNDARY_PLAN.md`. The recommended future boundary name is `pre-analysis-evidence-gate`; the future no-live contract/probe should stop product-photo-like synthetic filename/type hints before OCR and metadata processing while preserving receipt/PDF/screenshot behavior. Dev-only adapter harness work should wait until that gate contract/probe exists and passes.
...
- Phase 4.32 is complete as script/module-only OpenAI Vision sandbox skeleton hardening and fixture-runner validation in `docs/archive/phase-4/PHASE_4_32_VISION_SANDBOX_FIXTURE_RUNNER_VALIDATION.md`. It adds `src/lib/analysis/vision-sandbox/fixture-runner.ts`, `scripts/check-vision-sandbox-fixture-runner.cjs`, npm script `check:vision-sandbox-fixture-runner`, and semantic/boundary coverage. The runner validates all 12 approved synthetic fixture references, analysis-mode fallback behavior, unsupported/failure simulations, guard states, privacy/retention markers, and package-safety markers. No API route, route behavior change, OpenAI/provider SDK, env var, provider call, upload/storage/persistence, real evidence, fixture asset/metadata change, package artifact, UI wiring, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.33 is complete as OpenAI Vision sandbox developer usage and readiness checkpoint documentation in `docs/archive/phase-4/PHASE_4_33_VISION_SANDBOX_DEVELOPER_USAGE_READINESS.md`. It documents safe developer commands, current sandbox capabilities, non-capabilities, fixture/metadata rules, safety language, protected route/runtime/receipt boundaries, package/downloadable safety, readiness for provider-configuration planning only, and stop conditions before any API-credit-using implementation. No provider configuration, OpenAI/provider SDK, env var, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.34 is complete as OpenAI Vision provider configuration planning-only documentation in `docs/archive/phase-4/PHASE_4_34_OPENAI_VISION_PROVIDER_CONFIG_PLAN.md`. It defines future provider configuration goals, explicit opt-in and disabled-by-default rules, API-credit usage boundaries, future `.env.example` guidance only, model/provider selection strategy, timeout/retry/rate/cost defaults, logging/payload-retention/privacy defaults, sandbox relationship, downloadable/self-hosted package safety, approval gates, and Phase 4.35 recommendations. No provider configuration, OpenAI/provider SDK, env var, env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.35 is complete as real OpenAI Vision sandbox implementation planning-only documentation in `docs/archive/phase-4/PHASE_4_35_OPENAI_VISION_SANDBOX_IMPLEMENTATION_PLAN.md`. It defines the exact future implementation objective, future file-change boundaries, SDK versus minimal fetch adapter tradeoffs, provider request/response flow, synthetic-only first-call policy, API-credit approval policy, future env/config requirements, prompt/schema/fixture-runner integration, package/downloadable safety, future live-sandbox checks, Phase 4.36 recommendation, and stop conditions for the first API-credit-using phase. No live OpenAI implementation, provider configuration, OpenAI/provider SDK, env var, env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.36 is complete as the real OpenAI Vision sandbox approval checkpoint documentation in `docs/archive/phase-4/PHASE_4_36_OPENAI_VISION_SANDBOX_APPROVAL_CHECKPOINT.md`. It separates Robert's approval choices for provider configuration skeleton work, API-credit-using synthetic fixture calls, pausing live provider work, and downloadable/self-hosted setup planning; defines work allowed without API-credit approval, work blocked without API-credit approval, first-live-call rules, stop/rollback conditions, and package/downloadable safety; and recommends Phase 4.37 provider configuration skeleton implementation with no provider calls and no API-credit usage unless Robert explicitly approves API-credit use. No live OpenAI implementation, provider configuration, OpenAI/provider SDK, env var, env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, runtime schema/type migration, fixture asset/metadata change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.37 is complete as OpenAI Vision provider configuration skeleton implementation in `docs/archive/phase-4/PHASE_4_37_OPENAI_VISION_PROVIDER_CONFIG_SKELETON.md`. It adds inert sandbox-only provider config defaults in `src/lib/analysis/vision-sandbox/provider-config.ts`, exports them through the sandbox index, validates disabled defaults and unsafe candidate blocking through `vision-sandbox.probe.ts`, and extends semantic/boundary checks for provider-disabled, no-call, no-credit, payload-logging-disabled, raw-OCR-retention-disabled, synthetic-only evidence scope, and package-safe defaults. No `.env.example`, `.env`, `.env.local`, provider SDK, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, fixture asset/metadata change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.38 is complete as OpenAI Vision provider configuration skeleton hardening and `.env.example` / package-safety review in `docs/archive/phase-4/PHASE_4_38_OPENAI_VISION_PROVIDER_CONFIG_HARDENING.md`. It hardens unsafe candidate validation, adds a safe disabled `.env.example` with no secrets and no real values, expands boundary/semantic/probe coverage, preserves provider-disabled/request-execution-disabled/API-credit-disabled/payload-logging-disabled/raw-OCR-retention-disabled/synthetic-only defaults, and keeps package/downloadable safety explicit. No live OpenAI implementation, OpenAI/provider SDK, real env value, real env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, fixture change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change was added.
- Phase 4.39 is complete as provider configuration developer usage documentation and API-credit approval readiness in `docs/archive/phase-4/PHASE_4_39_PROVIDER_CONFIG_DEVELOPER_USAGE_AND_API_CREDIT_READINESS.md`. It documents safe `.env.example` usage, current disabled provider config defaults, allowed and blocked developer actions without Choice B, API-credit approval readiness, package/downloadable safety, first API-credit-using phase prerequisites, and the Phase 4.40 recommendation. It adds no live OpenAI implementation, OpenAI/provider SDK, real env value, real env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, fixture change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
- Phase 4.40 is complete as API-credit approval decision and first live sandbox call planning in `docs/archive/phase-4/PHASE_4_40_API_CREDIT_APPROVAL_AND_FIRST_LIVE_SANDBOX_CALL_PLAN.md`. It defines Choice B, records the recommended approval position, plans a future first synthetic-only provider call, selects `synthetic-clean-receipt-baseline` as the preferred first fixture, defines provider config/API-credit/output-validation/rollback/package-safety requirements, and recommends conditional Phase 4.41. It adds no live OpenAI implementation, OpenAI/provider SDK, real env value, real env file, secret, provider call, API-credit-using behavior, route behavior, upload/storage/persistence, UI wiring, fixture change, package artifact, real evidence, receipt behavior change, `analyzeEvidenceFile` change, or `LocalAnalysisResult` change.
...
- Phase 2.4.0 added the guarded non-live adapter readiness plan in `docs/archive/phase-2/PRODUCT_PHOTO_ADAPTER_READINESS_PLAN.md`. The plan defines future adapter contract boundaries, score/safety derivation rules, privacy limits, legacy `damage-photo` quarantine expectations, semantic/probe gates, protected live paths, and Phase 2.4.1 pass/fail criteria.
...
- `NEXT_STEPS.md`, `docs/archive/phase-2/PHASE_2_PHOTO_EVIDENCE_PLAN.md`, and `AGENT_LOG.md` for status updates.
...
- `NEXT_STEPS.md` and `AGENT_LOG.md` for status updates.
- `docs/archive/phase-2/PHASE_2_PHOTO_EVIDENCE_PLAN.md` only if durable host guidance needs a small update.
...
```text
/claimguardagent run Phase 4.41 only after Robert answers the Choice B question: if Robert explicitly approves Choice B in plain language, implement the first API-credit-using OpenAI Vision sandbox call for exactly one synthetic fixture (`synthetic-clean-receipt-baseline`) with disabled defaults, no routes, no uploads, no UI, no storage, no persistence, no real evidence, no fixture changes, no package artifacts, no analyzeEvidenceFile changes, no LocalAnalysisResult changes, no receipt behavior changes, no deployment, and full before/after checks; if Robert does not explicitly approve Choice B, run provider config/checker hardening only with no provider calls, no API-credit usage, no SDKs, no real env values, no secrets, and no runtime wiring
```
