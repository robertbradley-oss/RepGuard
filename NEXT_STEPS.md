...
```text
/claimguardagent run Phase 3.1 design system/case workflow concept planning after Phase 3.0 case workflow readiness; define the case review shell, evidence list/sidebar, selected evidence states, review summary panel, notes/manual decision panel, customer-safe wording panel, and timeline/history area; do not implement routes/components/code, wire ClaimReviewWorkflow, add live unsupported-evidence UI, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, connect providers/storage/integrations/case queues/OCR/metadata, deploy, commit, or push
```
