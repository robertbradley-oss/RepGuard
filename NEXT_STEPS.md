...
```text
/claimguardagent implement Phase 4.16 mock adapter route integration skeleton only if Robert explicitly approves: prefer a separate synthetic-only mock provider route rather than changing POST /api/analysis/ocr; preserve the current exact fixture-key OCR route behavior; accept only allowlisted synthetic mock inputs; add route probes and semantic coverage; do not add live providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR; run all required checks and targeted scans; commit and push only if all gates pass
```
