...
```text
/claimguardagent plan Phase 4.35 real OpenAI Vision sandbox implementation only: document exact future file changes, adapter choice, provider-call boundaries, API-credit usage approval, synthetic-only first-call rules, timeout/cost/rate policies, package-default behavior, and checker updates; do not add SDKs, env vars, env files, provider calls, API-credit-using behavior, route behavior, uploads, storage, persistence, UI wiring, fixture changes, package artifacts, real evidence, analyzeEvidenceFile changes, LocalAnalysisResult changes, receipt behavior changes, or deployment
```
