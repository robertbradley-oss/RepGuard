...
```text
/claimguardagent run Phase 4.36 real OpenAI Vision sandbox implementation approval checkpoint only: ask Robert to explicitly approve whether API credits may be used and whether provider configuration implementation may begin; document chosen adapter option, approved fixture count, cost ceiling, timeout ceiling, retry policy, config-file scope, and stop conditions; do not add SDKs, env vars, env files, provider calls, API-credit-using behavior, route behavior, uploads, storage, persistence, UI wiring, fixture changes, package artifacts, real evidence, analyzeEvidenceFile changes, LocalAnalysisResult changes, receipt behavior changes, or deployment
```
