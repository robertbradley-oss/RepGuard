...
```text
/claimguardagent prepare Phase 4.28 synthetic fixture creation plan only: define which synthetic fixture files/assets Phase 4.29 may create, exact allowed file types/directories/naming/content rules, package-safety/license/origin/no-real-evidence/no-private-identifier/no-provider-payload/no-raw-OCR gates, QA checks, stop conditions, and Phase 4.29 recommendation; do not create fixture files/images yet, no SDKs/env vars/provider calls, no route/runtime/UI/receipt behavior changes, no package artifacts, no `analyzeEvidenceFile` or `LocalAnalysisResult` changes
```
