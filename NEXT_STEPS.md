...
```text
/claimguardagent run Phase 4.10 as provider abstraction planning only for future OCR/vision providers: design the provider adapter interface, provider result type, provider failure type, timeout behavior, cost metering fields, retention/logging rules, redaction boundary, prompt/output safety rules, and synthetic/mock adapter plan; do not add live providers, SDKs, env vars, provider abstraction implementation, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR
```
