...
```text
/claimguardagent run Phase 4.7 as a review-only safety/readiness checkpoint for the synthetic OCR route skeleton: inspect POST /api/analysis/ocr, its route probe, Phase 4.2/4.3 contracts, source-of-truth docs, protected runtime files, provider/env/package boundaries, privacy/unsafe wording scans, and receipt-behavior preservation; do not add providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR
```
