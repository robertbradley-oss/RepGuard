...
- Phase 2.8.2 is implemented locally as a dev-only unsupported-evidence review bridge at `src/app/dev/unsupported-evidence-review/page.tsx` with literal synthetic cases in `src/app/dev/unsupported-evidence-review/render-cases.ts`. The route is production-disabled with `notFound()`, unlinked, synthetic-only, and covered by semantic/import/privacy guards in `scripts/check-report-semantics.mjs`. It displays unsupported status, evidence type labels, manual-review guidance, customer-safe wording, confidence/uncertainty treatment, blocked output reasons, allowed next actions, internal notes, no-live markers, and external-verification-not-performed copy. It does not value-import/call the mapper, import the workflow boundary helper, wire `ClaimReviewWorkflow`, route `ProductPhotoReviewPanel`, change upload/report/parser/scoring/fixture behavior, touch `analyzeEvidenceFile` or `LocalAnalysisResult`, add providers/storage/integrations/case queues, run OCR/metadata, deploy, push, or start Phase 2.9.
...
```text
/claimguardagent review and, if approved, commit the Phase 2.8.2 dev-only unsupported-evidence review bridge implementation if checks pass; do not push, deploy, wire ClaimReviewWorkflow, add live unsupported-evidence UI, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, or start Phase 2.9
```
