...
10. Do not start Phase 2.9 implementation; Phase 2.9 is planning-only until Robert explicitly opens a runtime/UI slice.
...
```text
/claimguardagent plan Phase 2.9.0 docs-only unsupported-evidence live-opt-in readiness: define the narrowest future caller-boundary planning questions and blockers before any live UI/runtime work; do not implement, wire ClaimReviewWorkflow, route ProductPhotoReviewPanel, change upload/report/parser/scoring/fixtures, change receipt behavior, connect providers/storage/integrations/case queues, deploy, commit, or push
```
