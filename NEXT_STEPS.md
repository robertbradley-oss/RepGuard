...
1. Run a review-only Phase 2.5 closeout next: confirm both dev-only harnesses (`/dev/pre-analysis-evidence-gate` and `/dev/product-photo-adapter-readiness`) remain no-live, synthetic-only, unlinked, and production-disabled; that `buildPreAnalysisEvidenceGateDecision` and `prepareProductPhotoAdapterReadinessForDevOnlyBoundary` stay decision-only and unwired; and that product-photo runtime, upload, UI, live report mapping, the live receipt analyzer entrypoint, the receipt-shaped result, providers, storage, integrations, and case queues remain blocked. Do not open runtime-facing (Phase 2.6) work without explicit approval.
