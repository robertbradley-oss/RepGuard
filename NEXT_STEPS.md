...
```text
/claimguardagent run Phase 4.9 as provider selection planning only for future OCR: compare provider-neutral options, privacy/data-flow prerequisites, cost/latency/failure considerations, receipt-regression gates, auth/rate-limit prerequisites, and no-live-provider stop conditions; do not add providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, analyzeEvidenceFile changes, receipt behavior changes, deployment, or live OCR
```
