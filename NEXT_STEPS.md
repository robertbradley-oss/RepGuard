...
```text
/claimguardagent implement Phase 4.29 synthetic fixture creation only: create the limited package-safe synthetic SVG/markdown fixtures planned in Phase 4.28 and matching Phase 4.27 metadata; no real/anonymized customer evidence, no external images, no provider calls, no SDKs/env vars, no upload/storage/persistence, no route/runtime/UI changes, no provider payloads/raw OCR dumps, no public/object URLs or storage handles, no package artifacts, no receipt behavior changes, no `analyzeEvidenceFile` or `LocalAnalysisResult` changes
```
