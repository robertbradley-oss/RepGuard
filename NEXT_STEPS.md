...
```text
/claimguardagent implement Phase 4.6 synthetic-only server-side OCR route skeleton only: add a mock-only JSON-only POST /api/analysis/ocr route that accepts allowlisted synthetic OCR fixture keys, uses the existing Phase 4.2 fixture harness and Phase 4.3 extraction contract, rejects real files, multipart data, object URLs, storage handles, provider payloads, and customer identifiers, returns review-support-only OCR extraction signals and safe failure states, adds route/probe/semantic/privacy coverage, and preserves ClaimReviewWorkflow, ProductPhotoReviewPanel, analyzeEvidenceFile, LocalAnalysisResult, receipt behavior, upload flow, providers, SDKs, env vars, storage, persistence, UI, deployment, and real evidence handling
```
