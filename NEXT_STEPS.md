...
```text
/claimguardagent commit the docs-only Phase 2.7.2 unsupported-evidence display/mapping probe plan if the diff is still limited to documentation and docs-safe checks pass; do not push, deploy, edit runtime code, add unsupported-evidence UI, wire the wrapper into live callers, route ProductPhotoReviewPanel, or change receipt behavior
```
