...
- Do not implement live OCR/provider work after Phase 4.3 without a separate approved provider, route, privacy, and retention slice.
...
```text
/claimguardagent run Phase 4.4 server-side OCR route and data-flow planning only: document privacy, retention, deletion, provider payload, timeout, cost, auth/storage prerequisites, and failure handling for a future route without implementing routes, providers, SDKs, env vars, uploads, storage, persistence, real evidence processing, AI/Vision calls, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, LocalAnalysisResult migration, receipt behavior changes, deployment, commit, or push unless explicitly approved
```
