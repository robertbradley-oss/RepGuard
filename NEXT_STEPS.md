...
1. Run Phase 2.5.1 only after Robert explicitly opens it: a gate-first, no-live, dev-only review harness implementation per `PHASE_2_5_DEV_HARNESS_SCOPE_PLAN.md` (literal synthetic `pre-analysis-evidence-gate` decision cases on a production-disabled, unlinked `/dev/pre-analysis-evidence-gate` route with type-only imports), or a review-only Phase 2.5 closeout. Either path keeps `buildPreAnalysisEvidenceGateDecision` decision-only and unwired and does not touch upload, UI, live report mapping, the live receipt analyzer entrypoint, the receipt-shaped result, providers, storage, integrations, case queues, or runtime.
