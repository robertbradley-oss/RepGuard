...
```text
/claimguardagent review and, if approved, commit the Phase 2.7.3 non-live unsupported-evidence mapper probe if the diff is limited to the isolated mapper/probe, semantic guard, probe runner, and status docs and all checks pass; do not push, deploy, add rendered unsupported-evidence UI, wire the wrapper into live callers, route ProductPhotoReviewPanel, or change receipt behavior
```
