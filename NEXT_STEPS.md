...
```text
/claimguardagent implement Phase 4.12 as a mock provider adapter skeleton only if approved: add an isolated synthetic/mock adapter under src/lib/analysis/providers/ with probe coverage for mock OCR success, mock vision success, timeout, unavailable, malformed, unsupported, privacy flags, no env/network/provider SDKs, no upload/storage/object URLs, no protected runtime imports, unsafe wording, private identifier rejection, no LocalAnalysisResult migration, and no analyzeEvidenceFile changes; do not add live providers, SDKs, env vars, uploads, storage, persistence, UI wiring, real evidence handling, ClaimReviewWorkflow wiring, ProductPhotoReviewPanel routing, receipt behavior changes, deployment, or live OCR
```
