...
- The recommended next safe task is Phase 4.35 real OpenAI Vision sandbox implementation planning only, focused on exact file changes, adapter choice, provider-call boundaries, API-credit usage approval, synthetic-only first-call rules, timeout/cost/rate policies, package-default behavior, and checker updates. Do not start live OpenAI Vision implementation unless Robert explicitly approves API-credit-using work.
