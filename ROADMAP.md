...
- The next safe options are Phase 4.21 OpenAI Vision sandbox schema planning only, Phase 4.21 OpenAI Vision sandbox test-fixture policy planning only, or Phase 4.21 OpenAI Vision sandbox skeleton implementation planning with no SDK/env/provider calls. Live OCR/provider work, real evidence handling, SDKs, env vars, uploads, storage, persistence, UI wiring, receipt behavior changes, and `LocalAnalysisResult` migration remain blocked unless Robert explicitly opens a named implementation slice.
