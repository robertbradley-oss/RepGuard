...
- Phase 4.38 documents and implements provider-config hardening in `PHASE_4_38_OPENAI_VISION_PROVIDER_CONFIG_HARDENING.md`, strengthens unsafe candidate validation, adds a safe disabled `.env.example` with no secrets and no real values, and expands probe/semantic/boundary coverage for package/downloadable safety.
...
- Phase 4.39 documents safe `.env.example` usage, the current disabled provider config skeleton, allowed and blocked developer actions without Choice B, API-credit approval readiness, package/downloadable safety, and first API-credit-using phase prerequisites in `PHASE_4_39_PROVIDER_CONFIG_DEVELOPER_USAGE_AND_API_CREDIT_READINESS.md`.
...
- Phase 4.40 documents Choice B as the separate approval required before any API-credit-using OpenAI Vision sandbox implementation, recommends asking Robert for that explicit decision, and defines a synthetic-only first-call plan in `PHASE_4_40_API_CREDIT_APPROVAL_AND_FIRST_LIVE_SANDBOX_CALL_PLAN.md`.
