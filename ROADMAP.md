...
- The next safe task is Phase 4.11 mock provider adapter planning only, or a mock adapter skeleton only if Robert explicitly approves implementation. Live OCR/provider work remains blocked unless Robert explicitly opens a named live/provider implementation slice.
