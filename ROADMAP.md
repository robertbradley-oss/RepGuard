...
- The next safe options are Phase 4.25 OpenAI Vision sandbox validation/probe implementation planning with no implementation, Phase 4.25 synthetic fixture metadata schema implementation planning with no runtime schema/types and no fixture files/images, or Phase 4.25 downloadable package safety planning for sandbox/demo data only. Live OCR/provider work, real evidence handling, SDKs, env vars, uploads, storage, persistence, UI wiring, receipt behavior changes, and `LocalAnalysisResult` migration remain blocked unless Robert explicitly opens a named implementation slice.
