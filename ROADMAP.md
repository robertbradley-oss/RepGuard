...
- The recommended next safe task is Phase 4.34 OpenAI Vision provider configuration planning only, with API-credit usage policy, provider opt-in, `.env.example` guidance without secrets, timeout/cost limits, package-safe disabled defaults, and developer-only provider boundaries. Do not start live OpenAI Vision implementation unless Robert explicitly approves API-credit-using work.
