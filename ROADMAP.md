...
- The next safe options are Phase 4.23 OpenAI Vision sandbox validation/probe planning only, Phase 4.23 OpenAI Vision sandbox skeleton implementation planning with no SDK/env/provider calls, or Phase 4.23 synthetic fixture metadata schema planning only with no fixture files/images. Live OCR/provider work, real evidence handling, SDKs, env vars, uploads, storage, persistence, UI wiring, receipt behavior changes, and `LocalAnalysisResult` migration remain blocked unless Robert explicitly opens a named implementation slice.
