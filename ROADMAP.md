...
- Phase 2.8.2 implements the dev-only unsupported-evidence review bridge locally at `/dev/unsupported-evidence-review` using `src/app/dev/unsupported-evidence-review/page.tsx` and route-local literal `UnsupportedEvidenceReviewDisplay`-shaped cases in `render-cases.ts`. The route is unlinked, production-disabled, synthetic-only, manually review oriented, and guarded by `scripts/check-report-semantics.mjs`; it does not call the mapper, import the workflow boundary helper, wire `ClaimReviewWorkflow`, route `ProductPhotoReviewPanel`, change upload/report/parser/scoring/fixture behavior, alter `analyzeEvidenceFile` or `LocalAnalysisResult`, add providers/storage/integrations/case queues, run OCR/metadata, or start Phase 2.9.
