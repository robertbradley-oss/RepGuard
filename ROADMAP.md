...
- The next safe task is Phase 4.13 review-only mock provider adapter safety/readiness checkpoint. Live OCR/provider work, route wiring, real evidence handling, SDKs, env vars, uploads, storage, persistence, UI wiring, receipt behavior changes, and `LocalAnalysisResult` migration remain blocked unless Robert explicitly opens a named implementation slice.
