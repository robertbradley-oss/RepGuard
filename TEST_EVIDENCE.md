...
Home Depot field extraction is source-scoped. Home Depot / Home Depot Canada layouts can pair `Online Order`, `Order #`, or `Order Number` labels with adjacent order-token rows, select `Order Total`, `Pickup Order Total`, `Delivery Total`, or `Refund Total` rows as the local total when visible, and pair `Payment`, `Payment Method`, or `Tender` labels with adjacent card/tender wording. A blank order label should not cause pickup, delivery, payment, subtotal, tax, or total text to be treated as an order number.
